...
package io.gamerope.monero.data;


import android.util.Log;
import io.gamerope.monero.model.NetworkType;
import io.gamerope.monero.model.WalletManager;
...
public class Node {
    ...
    Node(String nodeString) {
        ...
        if (daParts.length == 3) {
            try {
                ...
                Log.w(TAG, ex); // if we can't encode it, we don't use it
            }
        }
        this.name = name;
    ...
    public String toString() {
        ...
        if (name != null)
            try {
                ...
                Log.w(TAG, ex); // if we can't encode it, we don't store it
            }
        return sb.toString();
    }
    ...
    static public int getDefaultLevinPort() {
        ...
        switch (WalletManager.getInstance().getNetworkType()) {
            case NetworkType_Mainnet:
                ...
                break;
            case NetworkType_Testnet:
                DEFAULT_LEVIN_PORT = 22222;
                break;
            case NetworkType_Stagenet:
                DEFAULT_LEVIN_PORT = 33333;
                break;
            default:
    ...
    static public int getDefaultRpcPort() {
        ...
        switch (WalletManager.getInstance().getNetworkType()) {
            case NetworkType_Mainnet:
                DEFAULT_RPC_PORT = 11112;
                break;
            case NetworkType_Testnet:
                DEFAULT_RPC_PORT = 22223;
                break;
            case NetworkType_Stagenet:
                DEFAULT_RPC_PORT = 33334;
                break;
            default:
                throw new IllegalStateException("unsupported net " + WalletManager.getInstance().getNetworkType());
        }
