    ...
    public void draw(@NonNull Canvas canvas) {
        if (mBgColor != null) {
            if (mBgColor.length == 1) {
                mNewPaint.setColor(mBgColor[0]);
            } else {
                mNewPaint.setShader(new LinearGradient(mRect.left, mRect.height() / 2, mRect.right, mRect.height() / 2, mBgColor,
        ...
        if (mShape == SHAPE_ROUND) {
            ...
            canvas.drawRoundRect(mRect, mShapeRadius, mShapeRadius, mNewPaint);
        } else {
            canvas.drawCircle(mRect.centerX(), mRect.centerY(), Math.min(mRect.width(), mRect.height()) / 2, mPaint);
            canvas.drawCircle(mRect.centerX(), mRect.centerY(), Math.min(mRect.width(), mRect.height()) / 2, mNewPaint);
