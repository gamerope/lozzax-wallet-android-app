...
import io.gamerope.monero.data.Node
import io.gamerope.monero.data.TxData
import io.gamerope.monero.model.*
import io.gamerope.monero.util.RestoreHeight
...
object XMRWalletController {
    ...
    fun createWallet(aFile: File, password: String): io.gamerope.wallet.data.entity.Wallet {
    ...
    fun recoveryWallet(
        ...
        mnemonic: String,
        restoreHeight: Long
    ): io.gamerope.wallet.data.entity.Wallet {
        val newWallet = WalletManager.getInstance()
    ...
    private fun close(success: Boolean, newWallet: Wallet): io.gamerope.wallet.data.entity.Wallet {
        try {
            if (!success) {
                throw RuntimeException(newWallet.errorString)
            } else {
                val sym = sharedPreferences().getString("symbol", "") ?: ""
                return io.gamerope.wallet.data.entity.Wallet().apply {
                    symbol = sym
                    name = newWallet.name
    ...
    fun getTransactionHistory(): List<io.gamerope.wallet.data.entity.TransactionInfo> {
        val wallet = getWallet() ?: return emptyList()
        val all = wallet.history.all.sortedByDescending { it.timestamp }
        val allInfo = mutableListOf<io.gamerope.wallet.data.entity.TransactionInfo>()
        all.forEach {
    ...
    private fun convert(it: TransactionInfo): io.gamerope.wallet.data.entity.TransactionInfo {
        return io.gamerope.wallet.data.entity.TransactionInfo().apply {
    ...
    fun testRpcService(url: String, timeout: Int = 3 * 1000): Long {
        ...
        if (connection.responseCode == HttpURLConnection.HTTP_OK) {
            ...
            Log.e("gamerope", "$url--->>> height: ${header.optLong("height")}")
            Log.e("gamerope", "$url--->>> majorVersion: ${header.optLong("major_version")}")
        }
