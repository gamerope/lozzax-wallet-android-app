...
import io.gamerope.wallet.data.entity.*
import io.gamerope.wallet.data.remote.RetrofitManager
import io.gamerope.wallet.data.remote.entity.Empty
import io.gamerope.wallet.data.remote.entity.ExchangeAmount
import io.gamerope.wallet.data.remote.entity.RPCRequest
import io.gamerope.wallet.data.remote.entity.SwapCreateTransaction
import io.gamerope.wallet.support.extensions.unWrap
            ...
            try {
                ...
                minFromAmount.postValue(
                    RetrofitManager.changellyService.getMinAmount(
                        RPCRequest(
                            method = "getMinAmount",
                            params = mapOf("from" to "lozz", "to" to toCoin)
                        )
                    ).unWrap()
                )
                ...
                exchangeAmount.postValue(
                    RetrofitManager.changellyService.getExchangeAmount(
                        RPCRequest(
                            ...
                            params = listOf(mapOf("from" to "lozz", "to" to toCoin, "amount" to "1"))
                        )
                    ).unWrap()?.firstOrNull()
                )
            } catch (e: Exception) {
